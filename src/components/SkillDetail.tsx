...
export default function SkillDetail({ skill, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div
        className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-slate-800 border border-slate-700/50 rounded-2xl shadow-2xl animate-fade-in"
      ...
      >
        <div className="sticky top-0 flex items-center justify-between p-5 border-b border-slate-700/50 bg-slate-800/95 backdrop-blur-sm rounded-t-2xl z-10">
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-400 mt-0.5">{skill.github?.repo || skill.source}</p>
          </div>
          <button
            onClick={onClose}
